...
RUN apt-get install -y tmux  # for the new make console command
...
RUN cd / && git clone --depth=1 https://github.com/linux-kernel-labs/linux.git
RUN cd /linux/tools/labs && make -j $(nproc) zImage
RUN cd /linux/tools/labs && make rootfs
